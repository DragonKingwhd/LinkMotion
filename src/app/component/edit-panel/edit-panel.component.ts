...
export class EditPanelComponent implements OnInit, AfterContentInit, OnDestroy {
  @ViewChild('trajCSVInput') trajCSVInput!: ElementRef<HTMLInputElement>;

  optimizeForm = new FormBuilder().group({
    searchRadius: ['5'],
  });
  ...
  trajCSVInputClick(): void {
    this.trajCSVInput?.nativeElement?.click();
  }

  startTrajectoryDraw(): void {
    const jointId = this.activeSrv.selectedJoint.id;
    this.trajectoryService.startDrawing(jointId);
    NewGridComponent.instance.enterTrajectoryDrawMode();
  }

  importTrajectoryCSV(event: any): void {
    const file = event.target.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => {
      this.trajectoryService.importFromCSV(
        this.activeSrv.selectedJoint.id,
        reader.result as string
      );
      NewGridComponent.sendNotification('已导入 CSV 轨迹数据');
    };
  ...
  clearTrajectory(): void {
    this.trajectoryService.clearTrajectory(this.activeSrv.selectedJoint.id);
    NewGridComponent.sendNotification('已清除目标轨迹');
  }

  getTrajectoryPointCount(): number {
    const traj = this.trajectoryService.getTrajectory(this.activeSrv.selectedJoint?.id);
    return traj ? traj.pointCount : 0;
  }

  isOptimizing(): boolean {
    return this.optimizerService.isRunning$.value;
  }

  getOptimizationProgress(): any {
    return this.optimizerService.progress$.value;
  }

  startOptimization(): void {
    const radius = parseFloat(this.optimizeForm.value.searchRadius || '5') || 5;
    this.optimizerService.startOptimization(this.activeSrv.selectedJoint.id, {
      searchRadius: radius,
    });
  }
