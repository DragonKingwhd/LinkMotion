...
export class ToolbarComponent implements OnInit, AfterViewInit {
  ...
  upload($event: any) {
    ...
    if (input.files.length !== 1) {
      ...
      NewGridComponent.sendNotification('未选择文件');
      return;
    }
    ...
    reader.onload = () => {
      ...
      NewGridComponent.sendNotification('已从文件加载机构');
  ...
  alertNotAvailable() {
    ...
    NewGridComponent.sendNotification('此功能尚未开放');
  }
  ...
  handleUndo() {
    NewGridComponent.sendNotification('已撤销', 0);
  ...
  handleRedo() {
    NewGridComponent.sendNotification('已重做', 0);
