...
export class NewGridComponent {
  ...
  updateContextMenuItems() {
    ...
    switch (this.lastRightClick.constructor.name) {
      case 'Force':
        ...
        this.cMenuItems.push(
          new cMenuItem(
            (this.lastRightClick as Force).local ? '设为全局力' : '设为局部力',
      ...
      case 'RevJoint':
        ...
        this.cMenuItems.push(
          new cMenuItem('附加连杆', this.startCreatingLink.bind(this), 'new_link')
        );

        this.cMenuItems.push(
          new cMenuItem(
            jointIsGround ? '取消固定' : '设为固定',
        ...
        this.cMenuItems.push(
          new cMenuItem(
            this.gridUtils.isAttachedToSlider(this.lastRightClick) ? '移除滑块' : '添加滑块',
        ...
        this.cMenuItems.push(
          new cMenuItem(
            (this.lastRightClick as RealJoint).isWelded ? '取消焊接' : '焊接关节',
      ...
      case 'String': //This means grid
        this.cMenuItems.push(
          new cMenuItem('添加连杆', this.startCreatingLink.bind(this), 'new_link')
        );
    }
  }
  ...
  enterTrajectoryDrawMode(): void {
    this.gridStates = gridStates.drawTrajectory;
    const canvas = document.getElementById('canvas');
    if (canvas) {
      canvas.style.cursor = 'crosshair';
    }
    NewGridComponent.sendNotification('在画布上拖拽绘制目标轨迹，松开完成');
  }

  exitTrajectoryDrawMode(): void {
    const canvas = document.getElementById('canvas');
    if (canvas) {
      canvas.style.cursor = '';
    }
  ...
  mouseMove($event: MouseEvent) {
    ...
    switch (this.gridStates) {
      ...
      case gridStates.createJointFromLink:
        ...
        break;
      case gridStates.drawTrajectory:
        if (this.isMouseDown && this.trajectoryService.drawingJointId) {
          this.trajectoryService.addPoint(this.trajectoryService.drawingJointId, mousePosInSvg);
        }
        break;
    }
  ...
  mouseUp($event: MouseEvent) {
    ...
    if (this.gridStates === gridStates.drawTrajectory) {
      this.trajectoryService.stopDrawing();
      this.exitTrajectoryDrawMode();
    }
  ...
  onKeyPress($event: KeyboardEvent) {
    ...
    if ($event.keyCode == 27) {
      ...
      if (this.gridStates === gridStates.drawTrajectory) {
        ...
        this.exitTrajectoryDrawMode();
        this.gridStates = gridStates.waiting;
        NewGridComponent.sendNotification('已取消轨迹绘制');
        return;
      }
      this.activeObjService.updateSelectedObj(undefined);
    }
    ...
    if ($event.keyCode == 46) {
      ...
      if (true) {
        ...
        if (this.activeObjService.objType === 'Grid') {
          NewGridComponent.sendNotification('请选择要删除的对象。');
          return;
        }
        ...
        this.activeObjService.updateSelectedObj(undefined);
        NewGridComponent.sendNotification('已删除选中对象。');
      } else {
        return;
      }
    }
